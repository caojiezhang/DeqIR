...
def generalized_steps_sr(x, seq, model, Apy, A, Ap, b, logger=None, print_logs=False, **kwargs):
    bsz = x.size(0)
    seq_next = [-1] + list(seq[:-1])
    x0_preds = []
    xs = [x]
    ...
    for i, j in zip(reversed(seq), reversed(seq_next)):
        ...
        x0_t = (xt - et * (1 - at).sqrt()) / at.sqrt()
        # Eq. 17
        x0_t_hat = x0_t - Ap(A(x0_t)) + Apy   #x0_t - Ap(A(x0_t) - y)

        x0_preds.append(x0_t_hat)
        c1 = (
            kwargs.get("eta", 0) * ((1 - at / at_next) * (1 - at_next) / (1 - at)).sqrt()
        )
        c2 = ((1 - at_next) - c1 ** 2).sqrt()
        noise_t = torch.randn_like(x)
        xt_next = at_next.sqrt() * x0_t_hat + c1 * noise_t + c2 * et
        xs.append(xt_next)
    return xs, x0_preds
