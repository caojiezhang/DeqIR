...
def compute_multi_step_sr(xt, all_xT, model, et_coeff1, et_coeff2, et_prevsum_coeff, T, t, image_dim, xT, sf, y_coeff, all_y, A, Ap, noise_coeff, all_noise, use_svd, **kwargs):   
    with torch.no_grad():
...
def fp_implicit_iters_anderson(x, model, b, args=None, additional_args=None, logger=None, print_logs=False, save_last=True, max_iter=15, **kwargs):
    with torch.no_grad():   
        x0_preds = []
        image_dim = x.shape
        ...
        x_final = anderson(compute_multi_step_sr, all_xt, args['X'], args['F'], args['H'], args['y'], 
                                 additional_args, m=args['m'], lam=1e-3, max_iter=max_iter, tol=1e-3, beta=1.0, logger=logger)    #TODO 
